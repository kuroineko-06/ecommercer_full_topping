...
exports.verifyToken = async function (req, res) {
  try {
    ...
    const token = await Token.fineOne({ accessToken });
...
exports.verifyPaswordResetOTP = async function (req, res) {
  try {
    ...
    if (
      ...
      return res.status(401).json({ message: "Invalid or exprired OTP!" });
    }
...
exports.resetPassword = async function (req, res) {
  ...
  try {
    ...
    if (user.resetPasswordOtp !== 1) {
      return res
        .status(401)
        .json({ message: "Confirm OTP before reseting password" });
    }
